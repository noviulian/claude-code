...
./sync-to-global.sh
```
This backs up existing global config, syncs skills/agents/settings, and configures the statusline.
...
### Set up statusline only (macOS and Linux)
```bash
...
### Authentication
Claude Code handles authentication automatically. Use `claude login` to authenticate.
